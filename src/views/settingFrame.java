...
import controllers.giaController;
...
public class settingFrame extends javax.swing.JFrame {
    ...
    public void init(){
        setTitle("Setting");
        giaCTL.loadComboBox(giaComboBox);
        giaCTL.loadComboBox(giaLoaiMoiCB);
        giaCTL.loadComboBox(giaLoaiCB);
        maGiaMoiField.setText("G"+String.valueOf(giaCTL.layMaCuoi()));
        hienThiMoTa();
...
//    ///Panel GIA
    ...
    public void xoaFunc(){
        ...
        giaCTL.xoaGia(item);
        refresh();
        JOptionPane.showMessageDialog(rootPane, "Đã xóa giá "+ item, "Message", 1);
    }
    //Ham xu ly nut them
    public void themFunc(){
        Gia giaThem = new Gia(maGiaMoiField.getText(),moTaMoiField.getText());
        try {
            giaCTL.themGia(giaThem);
            refresh();
        } catch (Exception e) {
            System.err.println("Khong them duoc gia!");
        }
    }
    //Ham xu ly nut cap nhap
    public void capNhapFunc(){
        Gia giaCN = new Gia((String) giaComboBox.getSelectedItem(),motaField.getText());
        try {
            giaCTL.capNhapGia(giaCN);
            JOptionPane.showMessageDialog(rootPane, "Đã cập nhập giá "+ giaCN.getMaGia(), "Message", 1);
            refresh();
        } catch (Exception e) {
...
/// Panel Loai
    ...
    public void loaiSelectedFunc(){
        if(loaiList.getSelectedValue()!= null){
            ...
            giaLoaiCB.setSelectedItem(loai_list[indexC_loai].getMaGia());
        }
    }
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("Giá", jPanel1);
